...
u8 source_buffer[16384];
u8 dest_buffer[16384];
	...
	*/
	while (!kthread_should_stop() &&
		...
		*/
		...
		/*
		// Poll util the transmit transaction complete.
		*/
	...
	*/
	...
	ktime = ktime_get();
		...
		*/
		...
		dma_srcs = dma_map_single(tx_dev->dev, source_buffer, test_buf_size,
						     DMA_MEM_TO_DEV);
		...
		*/
		...
		if (!rxd || !txd) {
			dma_unmap_single(tx_dev->dev, dma_srcs, test_buf_size,
						 DMA_MEM_TO_DEV);
